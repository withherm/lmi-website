...
## EQUIBT COHERENCE REALIGNMENT (Herman, 2026-07-13) — THIS BLOCK GOVERNS ABOVE ALL BELOW

Decided after equibt.com went live on the fully hand-drawn v6x system. Herman's call: pull LMI back into visual coherence with EQUIBT so the two read as one family, sharing the hand-drawn line-art idiom, type, sizes, UI and voice, and differentiating on **colour and photography only**. This **overrides the 2026-06-11 "DIRECTION CHANGE — visual idiom" block** (structured/Forma, hand-drawn only in belt badges) and the **all-Montserrat rule in the 2026-06-12 REVERSAL block**. Where any text below says "structured/formal system", "Forma covers program/sector/feature icons", "hand-drawn only in the belt badges", or "all-Montserrat / no Lora", read it as superseded here. The crimson-led colour system and the photography survive and become LMI's two differentiators. (Trade-off, on the record: this gives up the earlier "a hand-drawn wobble contradicts Lean precision / de-risks the conservative buyer" argument. Herman weighed it and chose family coherence. Keep the line clean and architectural, not scrappy, so precision still reads.)

**1. Icons + idiom = EQUIBT hand-drawn line-art (reverses structured/Forma).** LMI adopts EQUIBT's hand-drawn line-art system wholesale. Heroes and content glyphs (programs/pillars, sectors, feature/difference cards) are hand-drawn **stroke** art: **charcoal line ink `#22242A`** (navy dropped from LMI, 2026-07-13), ~2px stroke, round caps, each with ONE **crimson `#A51C30`** accent sub-shape (crimson is LMI's accent where EQUIBT uses terracotta, same treatment). The portfolio drawing set is shared with EQUIBT, same geometry, recoloured charcoal line + crimson accent instead of navy line + terracotta. Forma Regular is demoted to the functional/utility layer + fallback for any new glyph with no hand-drawn equivalent, exactly as on EQUIBT. Production path mirrors EQUIBT: Herman draws in Affinity, Claude normalises to navy line + one crimson accent, flattens for Safari, wraps as `.astro` via a `HERO_ICON_HANDOFF`-style JSON. Shared portfolio drawing set with EQUIBT: same glyphs, crimson accent instead of terracotta. The belt/credential badges keep their hand-drawn squircle, now consistent with a fully hand-drawn system rather than the sole exception.

**2. Type = Montserrat + Lora (reverses all-Montserrat).** LMI runs the EQUIBT pairing: Montserrat 700 headings (charcoal `#2C3E50`, never crimson), **Lora 500 body 19px** `#161616`, Lora 500 **22px** hero subhead/lede only, italic Lora captions, Montserrat for eyebrows/labels/nav/chips. Scaffold: add `@fontsource/lora` alongside `@fontsource/montserrat`; update the `--font-*` tokens. The "LMI differentiates on fonts" logic is retired: LMI differentiates on colour + photography only.

**3. Colour = charcoal + crimson, a clean two-colour system (refined 2026-07-13, Herman) and LMI's primary differentiator.** Crimson `#A51C30` is the only accent (buttons, links + underline, eyebrows text + dot, chevrons, one accent per glyph, active-belt highlight), hover `#87121F`. **Charcoal `#22242A`** is the single structural ink: headings, structure AND the hand-drawn line-art (headings never crimson). Body near-black `#161616` (Lora). Grounds white + light grey. **Navy `#26245C` and cobalt are dropped from LMI entirely** (they read as EQUIBT); the hairline moves to a charcoal tint `rgba(34,36,42,0.14)` and the one dark-moment section is deep charcoal `#1A1B1E`, not ink navy. Mapping rule: **anywhere EQUIBT uses navy, LMI uses charcoal `#22242A`; anywhere EQUIBT uses terracotta, LMI uses crimson `#A51C30`** (same roles, LMI's two hues). Belt colours identify only. This matches Herman's live Squarespace LMI (charcoal headings, crimson accents).

**4. Sizes + UI = EQUIBT live tokens.** Match the shipped EQUIBT build: `--hairline` (EQUIBT navy tint `rgba(38,36,92,0.13)`; LMI uses a charcoal tint `rgba(34,36,42,0.14)` to stay navy-free), `--max:1180`, `--wide:1320`, `--hero-art-w:min(312px,34vw)`; 1px hairline cards (~16px radius, no shadow, hover = border darken + small lift); the `›` chevron-link mechanic (crimson underline L-to-R then chevron nudge); one fade + 8px section reveal, 350ms, once; sticky white nav ~80px; hero soft-orbit settle (the `hero-orbit.ts` pattern). **Inherit the v6x HARD NOs verbatim:** no accent top-bars or coloured rules on cards, no 01/02/03 numbering, no tint-on-hover, no gradient on featured cards, no fake `border-radius` squircle (belt badges are a true SVG superellipse), alternate grounds instead of hairline dividers.

**5. Photography = kept, alongside the line-art (Herman, 2026-07-13).** LMI keeps photography where EQUIBT has none. Default hero = the hand-drawn line-art motif (for coherence with EQUIBT); a treated/art-directed image is the alternative where a genuinely strong session photo exists. The three trust zones (faculty, in-house delivery proof, lead testimonial) stay **real** photography, never AI. Dosage rule holds: structure + line-art lead, photography is proof/atmosphere. This is LMI's second differentiator from EQUIBT.

**6. Voice = shared house rules, LMI register.** Firm "we" (never name Herman), verb-first active headlines, Australian English, no em/en dashes, no emoji. LMI tone stays a touch warmer than EQUIBT (expert, accessible, energising, vs authoritative). "Get in touch" CTA (async), matching EQUIBT. The EQUIBT ↔ LMI reciprocal handover blocks stay.

**Cascade:** (a) EQT-299 home rebuild redirects to this direction, see `Runtime/01_Projects/Active/SESSION_BRIEF_LMI_EQUIBT_COHERENCE.md`. (b) `BRAND_SYSTEM.md` LMI type + iconography rows updated to match (2026-07-13). (c) The "signature" is now the belt ladder + hand-drawn line-art + crimson discipline (the Cobalt-on-Pale-Sky framing is fully retired). (d) The bespoke LSS/Healthcare glyph redraw item now covers the whole hand-drawn icon set, drawn once and shared with EQUIBT.
...
> **STOCK-ICON SCOPE (locked 2026-06-13, Herman sign-off; revised later same day, supersedes the first "stays bespoke" cut).** **Icons8 Forma Regular** (one family, rounded; no Light/Thin/Bold/Sharp/two-tone/Material on production UI) covers **program icons, sector icons, feature/difference card icons AND all functional/utility glyphs** (nav, chevrons, close, search, hamburger, social, arrows, form affordances), plus internal collateral, decks and app/training UI. LMI recolours Forma to slate `#2C3E50` glyph via `fill:currentColor`, crimson `#A51C30` accent; public-site utility glyphs are small and single-colour. **Bespoke is now reserved for: brand marks, hero, genuine illustration, and the hand-drawn belt/credential badges (LMI's single craft token, unchanged).** Forma must not replace the hero or the belt badges. NOTE: this means program and sector icons move from bespoke EQUIBT line-art lifts to Forma; the precision idiom still holds (Forma is clean architectural line, no wobble, on-message for Lean). Shared portfolio set with EQUIBT (same icons, different colour). Catalogues in the GOS vault: `Docs/LMI/ARTWORK/ICONS8_FORMA/ICON_CATALOGUE.html` (full, weight toggle Light/Regular/Thin/Doodle) and `ICON_CATALOGUE_CORE.html` (curated 600).
